...
struct GPUAnimBitmap {
	...
	char* state1, *state2;
    void    *dataBlock;
    void (*fAnim)(uchar4*,void*,char*, char*, float *);
    void (*animExit)(void*);
    ...
    void anim_and_exit( void (*f)(uchar4*,void*,char*, char*, float*), void(*e)(void*) ) {
    ...
    static void idle_func( void ) {
		...
		curandGenerateUniform(bitmap->gen, bitmap->rands, bitmap->width*bitmap->height);
		if(++ticks%2==0)
			bitmap->fAnim( devPtr, bitmap->dataBlock, bitmap->state1, bitmap->state2, bitmap->rands);
		else
			bitmap->fAnim( devPtr, bitmap->dataBlock, bitmap->state2, bitmap->state1, bitmap->rands);
